 ...
 * @internal
 */
async function getContentType(data) {
    ...
    if (isArrayBuffer(data)) {
        ...
        // Blob
        bytes = await getFirstFourBytesFromBlob();
    }
    else {
 ...
 */
function elementReferenceToFormElement(ref, readResults) {
    ...
    if (!Number.isNaN(wordIndex) && elementKind === "lines") {
        return baseElement.words[wordIndex];
    }
    return baseElement;
}
/**
...
function toFieldData(pageNumber, original, readResults) {
    ...
    return {
        ...
        fieldElements: (_a = original.elements) === null || _a === void 0 ? void 0 : _a.map((element) => elementReferenceToFormElement(element, readResults))
    };
}
/**
 * @internal
    ...
    return {
        ...
        cells: original.cells.map((cell) => {
            ...
            return ({
                ...
                fieldElements: (_a = cell.elements) === null || _a === void 0 ? void 0 : _a.map((element) => elementReferenceToFormElement(element, readResults)),
...
function toFormFieldFromFieldValueModel(original, key, readResults) {
    ...
    return {
        ...
        valueData: {
            ...
            text: original.text,
            boundingBox: original.boundingBox ? toBoundingBox(original.boundingBox) : undefined,
            fieldElements: (_c = original.elements) === null || _c === void 0 ? void 0 : _c.map((element) => elementReferenceToFormElement(element, readResults))
        },
...
class FormRecognizerClient {
     ...
     */
    async beginRecognizeContent(form, options = {}) {
        const client = {
     ...
     */
    async beginRecognizeContentFromUrl(formUrl, options = {}) {
        const client = {
            beginRecognize: (...args) => recognizeLayoutInternal(this.client, ...args),
            getRecognizeResult: (...args) => this.getRecognizedContent(...args)
        };
     ...
     * @internal
     */
    async getRecognizedContent(resultId, options) {
     ...
     */
    async beginRecognizeCustomForms(modelId, form, options = {}) {
        if (!modelId) {
        ...
        const poller = new FormRecognitionPoller(Object.assign({ modelId, createOperation: span("customFormsInternal", async (finalOptions) => {
                var _a;
                const requestBody = await toRequestBody(form);
                const contentType = (_a = finalOptions.contentType) !== null && _a !== void 0 ? _a : (await getContentType(requestBody));
                return processOperationLocation(await this.client.analyzeWithCustomModel(modelId, contentType, requestBody, coreHttp.operationOptionsToRequestOptionsBase(finalOptions)));
            ...
            modelIdParam, resultId, coreHttp.operationOptionsToRequestOptionsBase(finalOptions))) }, options));
        await poller.poll();
        return poller;
    }
    /**
     ...
     */
    async beginRecognizeCustomFormsFromUrl(modelId, formUrl, options = {}) {
        if (!modelId) {
            throw new RangeError("Invalid model id");
        }
        const { span } = makeSpanner("FormRecognizerClient-beginRecognizeCustomForms", Object.assign(Object.assign({}, options), { includeTextDetails: options.includeFieldElements }));
        const poller = new FormRecognitionPoller(Object.assign({ modelId, createOperation: span("customFormsInternal", async (finalOptions) => {
                return processOperationLocation(await this.client.analyzeWithCustomModel(modelId, "application/json", Object.assign({ fileStream: {
                        source: formUrl
     ...
     * @param options - Options for the recognition operation
     */
    async beginRecognizeBusinessCards(businessCard, options = { includeFieldElements: false }) {
        ...
        const poller = new FormRecognitionPoller(Object.assign({ expectedDocType: "prebuilt:businesscard", createOperation: span("businessCardsInternal", async (finalOptions) => {
                var _a;
                const requestBody = await toRequestBody(businessCard);
                const contentType = (_a = finalOptions.contentType) !== null && _a !== void 0 ? _a : (await getContentType(requestBody));
                return processOperationLocation(await this.client.analyzeBusinessCardAsync(contentType, requestBody, coreHttp.operationOptionsToRequestOptionsBase(finalOptions)));
            }), getResult: span("getBusinessCards", async (finalOptions, resultId) => this.client.getAnalyzeBusinessCardResult(resultId, coreHttp.operationOptionsToRequestOptionsBase(finalOptions))) }, options));
        await poller.poll();
        return poller;
    }
    /**
     ...
     * @param options - Options for the recognition operation
     */
    async beginRecognizeBusinessCardsFromUrl(businessCardUrl, options = { includeFieldElements: false }) {
        if (options.contentType) {
            logger.warning("Ignoring 'contentType' parameter passed to URL-based method.");
        }
        ...
        const poller = new FormRecognitionPoller(Object.assign({ expectedDocType: "prebuilt:businesscard", createOperation: span("businessCardsInternal", async (finalOptions) => {
                return processOperationLocation(await this.client.analyzeBusinessCardAsync("application/json", Object.assign({ fileStream: {
     ...
     * @param options - Options for the recognition operation
     */
    async beginRecognizeInvoices(invoice, options = {}) {
        ...
        const poller = new FormRecognitionPoller(Object.assign({ expectedDocType: "prebuilt:invoice", createOperation: span("invoicesInternal", async (finalOptions) => {
                var _a;
                const requestBody = await toRequestBody(invoice);
                const contentType = (_a = finalOptions.contentType) !== null && _a !== void 0 ? _a : (await getContentType(requestBody));
                return processOperationLocation(await this.client.analyzeInvoiceAsync(contentType, requestBody, coreHttp.operationOptionsToRequestOptionsBase(finalOptions)));
            }), getResult: span("getInvoices", async (finalOptions, resultId) => this.client.getAnalyzeInvoiceResult(resultId, coreHttp.operationOptionsToRequestOptionsBase(finalOptions))) }, options));
        await poller.poll();
        return poller;
    }
    /**
     ...
     * @param options - Options for the recognition operation
     */
    async beginRecognizeInvoicesFromUrl(invoiceUrl, options = {}) {
        if (options.contentType) {
            logger.warning("Ignoring 'contentType' parameter passed to URL-based method.");
        }
        ...
        const poller = new FormRecognitionPoller(Object.assign({ expectedDocType: "prebuilt:invoice", createOperation: span("invoicesInternal", async (finalOptions) => {
                return processOperationLocation(await this.client.analyzeInvoiceAsync("application/json", Object.assign({ fileStream: {
                        source: invoiceUrl
                    } }, coreHttp.operationOptionsToRequestOptionsBase(finalOptions))));
            }), getResult: span("getInvoices", async (finalOptions, resultId) => this.client.getAnalyzeInvoiceResult(resultId, coreHttp.operationOptionsToRequestOptionsBase(finalOptions))) }, options));
        await poller.poll();
     ...
     * @param options - Options for the recognition operation
     */
    async beginRecognizeIdentityDocuments(identityDocument, options = {}) {
        ...
        const poller = new FormRecognitionPoller(Object.assign({ expectedDocType: "prebuilt:idDocument", createOperation: span("identityDocumentsInternal", async (finalOptions) => {
                var _a;
                const requestBody = await toRequestBody(identityDocument);
                const contentType = (_a = finalOptions.contentType) !== null && _a !== void 0 ? _a : (await getContentType(requestBody));
                return processOperationLocation(await this.client.analyzeIdDocumentAsync(contentType, requestBody, coreHttp.operationOptionsToRequestOptionsBase(finalOptions)));
            }), getResult: span("getIdentityDocuments", async (finalOptions, resultId) => this.client.getAnalyzeIdDocumentResult(resultId, coreHttp.operationOptionsToRequestOptionsBase(finalOptions))) }, options));
        await poller.poll();
        return poller;
    }
    /**
     ...
     * @param options - Options for the recognition operation
     */
    async beginRecognizeIdentityDocumentsFromUrl(identityDocumentUrl, options = {}) {
        if (options.contentType) {
            logger.warning("Ignoring 'contentType' parameter passed to URL-based method.");
        }
        ...
        const poller = new FormRecognitionPoller(Object.assign({ expectedDocType: "prebuilt:idDocument", createOperation: span("identityDocumentsInternal", async (finalOptions) => {
                return processOperationLocation(await this.client.analyzeIdDocumentAsync("application/json", Object.assign({ fileStream: {
                        source: identityDocumentUrl
                    } }, coreHttp.operationOptionsToRequestOptionsBase(finalOptions))));
            }), getResult: span("getIdDocuments", async (finalOptions, resultId) => this.client.getAnalyzeIdDocumentResult(resultId, coreHttp.operationOptionsToRequestOptionsBase(finalOptions))) }, options));
        await poller.poll();
        return poller;
    }
    // #endregion
     ...
     * @param options - Options for the recognition operation
     */
    async beginRecognizeReceipts(receipt, options = {}) {
        ...
        const poller = new FormRecognitionPoller(Object.assign({ expectedDocType: "prebuilt:receipt", createOperation: span("receiptsInternal", async (finalOptions) => {
                var _a;
                const requestBody = await toRequestBody(receipt);
                const contentType = (_a = finalOptions.contentType) !== null && _a !== void 0 ? _a : (await getContentType(requestBody));
                return processOperationLocation(await this.client.analyzeReceiptAsync(contentType, requestBody, coreHttp.operationOptionsToRequestOptionsBase(finalOptions)));
            }), getResult: span("getReceipts", async (finalOptions, resultId) => this.client.getAnalyzeReceiptResult(resultId, coreHttp.operationOptionsToRequestOptionsBase(finalOptions))) }, options));
        await poller.poll();
        return poller;
    }
    /**
     ...
     * @param options - Options for the recognition operation
     */
    async beginRecognizeReceiptsFromUrl(receiptUrl, options = {}) {
        if (options.contentType) {
            logger.warning("Ignoring 'contentType' parameter passed to URL-based method.");
        }
        ...
        const poller = new FormRecognitionPoller(Object.assign({ expectedDocType: "prebuilt:receipt", createOperation: span("receiptsInternal", async (finalOptions) => {
                return processOperationLocation(await this.client.analyzeReceiptAsync("application/json", Object.assign({ fileStream: {
                        source: receiptUrl
                    } }, coreHttp.operationOptionsToRequestOptionsBase(finalOptions))));
            }), getResult: span("getReceipts", async (finalOptions, resultId) => this.client.getAnalyzeReceiptResult(resultId, coreHttp.operationOptionsToRequestOptionsBase(finalOptions))) }, options));
        await poller.poll();
...
client, body, contentType, options, _modelId) {
    ...
    try {
        if (requestContentType) {
            return await client.analyzeLayoutAsync(requestContentType, requestBody, coreHttp.operationOptionsToRequestOptionsBase(finalOptions));
        }
        return await client.analyzeLayoutAsync("application/json", Object.assign({ fileStream: requestBody }, coreHttp.operationOptionsToRequestOptionsBase(finalOptions)));
    }
    catch (e) {
        span.setStatus({
            code: coreTracing.SpanStatusCode.ERROR,
...
function makeBeginTrainingPollOperation(client, state) {
    return {
        ...
        async update(options = {}) {
            ...
            if (!pollerState.isStarted) {
                pollerState.isStarted = true;
                const result = await client.trainCustomModelInternal(trainingInputs, false, trainModelOptions || {});
                if (!result.location) {
                    throw new Error("Expect a valid 'operationLocation' to retrieve analyze results");
                }
...
function makeBeginCopyModelPollOperation(state) {
    return {
        ...
        async update(options = {}) {
            ...
            if (!pollerState.isStarted) {
                pollerState.isStarted = true;
                const result = await client.beginCopyModel(modelId, copyAuthorization, copyModelOptions || {});
                if (!result.operationLocation) {
                    throw new Error("Expect a valid 'operationLocation' to retrieve analyze results");
                }
                const lastSlashIndex = result.operationLocation.lastIndexOf("/");
...
class FormTrainingClient {
     ...
     */
    async getAccountProperties(options) {
        const realOptions = options || {};
        const { span, updatedOptions: finalOptions } = createSpan("FormTrainingClient-listCustomModels", realOptions);
        try {
            const result = await this.client.getCustomModels(Object.assign({}, coreHttp.operationOptionsToRequestOptionsBase(finalOptions)));
            return {
    ...
    async deleteModel(modelId, options) {
        ...
        try {
            return await this.client.deleteCustomModel(modelId, coreHttp.operationOptionsToRequestOptionsBase(finalOptions));
        }
        catch (e) {
            span.setStatus({
                code: coreTracing.SpanStatusCode.ERROR,
     ...
     */
    async getCustomModel(modelId, options = {}) {
        const realOptions = options || {};
        const { span, updatedOptions: finalOptions } = createSpan("FormTrainingClient-getCustomModel", realOptions);
        try {
            const response = await this.client.getCustomModel(modelId, Object.assign(Object.assign({}, coreHttp.operationOptionsToRequestOptionsBase(finalOptions)), { 
    ...
    async list(options) {
        ...
        try {
            const result = await this.client.listCustomModels(Object.assign({}, coreHttp.operationOptionsToRequestOptionsBase(finalOptions)));
            return result;
        }
        catch (e) {
            span.setStatus({
     ...
     * @hidden
     */
    async listNextPage(nextLink, options) {
        const realOptions = options || {};
        const { span, updatedOptions: finalOptions } = createSpan("FormTrainingClient-listNextPage", realOptions);
        try {
            const result = await this.client.listCustomModelsNext(nextLink, Object.assign({}, coreHttp.operationOptionsToRequestOptionsBase(finalOptions)));
            return result;
        }
        catch (e) {
            span.setStatus({
     ...
     */
    async beginTraining(trainingFilesUrl, useTrainingLabels, options = {}) {
     ...
     */
    async beginCreateComposedModel(modelIds, options) {
     ...
     */
    async getCopyAuthorization(resourceId, resourceRegion, options = {}) {
        const { span, updatedOptions: finalOptions } = createSpan("FormTrainingClient-getCopyAuthorization", options);
        try {
            const response = (await this.client.generateModelCopyAuthorization(coreHttp.operationOptionsToRequestOptionsBase(finalOptions)));
            return {
     ...
     */
    async beginCopyModel(modelId, target, options = {}) {
     ...
     * @hidden
     */
    async beginCopyModelInternal(modelId, copyAuthorization, options = {}) {
        const { span, updatedOptions: finalOptions } = createSpan("FormTrainingClient-beginCopyModelInternal", options);
        try {
            return await this.client.copyCustomModel(modelId, {
     ...
     * @hidden
     */
    async getCopyModelResult(modelId, resultId, options = {}) {
        const { span, updatedOptions: finalOptions } = createSpan("FormTrainingClient-getCopyModelResult", options);
        try {
            return await this.client.getCustomModelCopyResult(modelId, resultId, coreHttp.operationOptionsToRequestOptionsBase(finalOptions));
        }
        catch (e) {
            span.setStatus({
                code: coreTracing.SpanStatusCode.ERROR,
